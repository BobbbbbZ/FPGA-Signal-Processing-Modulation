...
extern struct gimp_image_struct image1;
extern struct gimp_image_struct image2;
extern struct gimp_image_struct image3;
...
void init_background() { // INIT BACKGROUND
	...
	//init lines
	vid_draw_line(652, 200, 795, 200, 4, WHITE_24, SW_Frame); // bet my button and mod
	...
	draw_button(667, 205, 120, 35, 0x0066CC, "ASK", 1, SW_Frame);
	draw_button(667, 245, 120, 35, 0x0066CC, "FSK", 0, SW_Frame);
	draw_button(667, 285, 120, 35, 0x0066CC, "BPSK", 0, SW_Frame);
	draw_button(667, 325, 120, 35, 0x0066CC, "LFSR", 0, SW_Frame);
	...
	draw_button(667, 50, 120, 35, 0x0066CC, "COLOR", 0, SW_Frame);
	draw_button(667, 100, 120, 35, 0x0066CC, "NEXT", 0, SW_Frame);
	draw_button(667, 150, 120, 35, 0x0066CC, "PREV", 0, SW_Frame);
...
;
audio_dac_wr_fifo( buf[i % size_song1]);
}
...
if (play_song == 1) {
	...
	else if (audio_select % 3 == 2) 
	{send_audio_fifo(&song3[address_counter % size_song3]);
		if (address_counter < size_song3) {
		address_counter += NUMERO_DE_MUESTRAS;
	} else {
		address_counter = 0;
	}
...
if (event == 1) {	//down event
	...
	if (x_mouse >= 667 && x_mouse <= (667 + 120) && y_mouse >= 100
			&& y_mouse <= (100 + 35)) {
		draw_button(667, 100, 120, 35, 0x0066CC, "NEXT", 1, SW_Frame);
		draw_button(667, 150, 120, 35, 0x0066CC, "PREV", 0, SW_Frame);
		address_counter = 0;
		next = 1;
		audio_select += 1;
	...
	else if (x_mouse >= 667 && x_mouse <= (667 + 120) && y_mouse >= 150
			&& y_mouse <= (150 + 35)) {
		draw_button(667, 100, 120, 35, 0x0066CC, "NEXT", 0, SW_Frame);
		draw_button(667, 150, 120, 35, 0x0066CC, "PREV", 1, SW_Frame);
		prev = 1;
		audio_select += 2;	
		address_counter = 0;
	}
	...
	if (audio_select % 3 == 0) {
			DispIMAGE_from_gimp(SW_Frame, &image1, 0, 0, BLACK_24);
	}
	else if (audio_select % 3 == 1) {
			DispIMAGE_from_gimp(SW_Frame, &image2, 0, 0, BLACK_24);
	}
	else if (audio_select % 3 == 2) {
			DispIMAGE_from_gimp(SW_Frame, &image3, 0, 0, BLACK_24);
	}

	if (x_mouse >= 667 && x_mouse <= (667 + 120) && y_mouse >= 50
			&& y_mouse <= (50 + 35)) {
		draw_button(667, 50, 120, 35, 0x0066CC, "COLOR", 0, SW_Frame);

		color_select = (color_select + 1) % 3;
		select_color(color_select);
	}

	//Logic for the modulation buttons
	if (x_mouse >= 667 && x_mouse <= (667 + 120) && y_mouse >= 205
			&& y_mouse <= (205 + 35)) {
			draw_button(667, 205, 120, 35, 0x0066CC, "ASK", 1, SW_Frame);
			draw_button(667, 245, 120, 35, 0x0066CC, "FSK", 0, SW_Frame);
			draw_button(667, 285, 120, 35, 0x0066CC, "BPSK", 0, SW_Frame);
	...
	else if (x_mouse >= 667 && x_mouse <= (667 + 120) && y_mouse >= 245
			&& y_mouse <= (245 + 35)) {
			draw_button(667, 205, 120, 35, 0x0066CC, "ASK", 0, SW_Frame);
			draw_button(667, 245, 120, 35, 0x0066CC, "FSK", 1, SW_Frame);
			draw_button(667, 285, 120, 35, 0x0066CC, "BPSK", 0, SW_Frame);
			draw_button(667, 325, 120, 35, 0x0066CC, "LFSR", 0, SW_Frame);
			draw_button(667, 365, 120, 35, 0x0066CC, "QPSK", 0, SW_Frame);
		select_modulation(1);
	} else if (x_mouse >= 667 && x_mouse <= (667 + 120) && y_mouse >= 285
			&& y_mouse <= (285 + 35)) {
			draw_button(667, 205, 120, 35, 0x0066CC, "ASK", 0, SW_Frame);
			draw_button(667, 245, 120, 35, 0x0066CC, "FSK", 0, SW_Frame);
			draw_button(667, 285, 120, 35, 0x0066CC, "BPSK", 1, SW_Frame);
			draw_button(667, 325, 120, 35, 0x0066CC, "LFSR", 0, SW_Frame);
			draw_button(667, 365, 120, 35, 0x0066CC, "QPSK", 0, SW_Frame);
		select_modulation(2);
	} else if (x_mouse >= 667 && x_mouse <= (667 + 120) && y_mouse >= 325
			&& y_mouse <= (325 + 35)) {
			draw_button(667, 205, 120, 35, 0x0066CC, "ASK", 0, SW_Frame);
			draw_button(667, 245, 120, 35, 0x0066CC, "FSK", 0, SW_Frame);
			draw_button(667, 285, 120, 35, 0x0066CC, "BPSK", 0, SW_Frame);
			draw_button(667, 325, 120, 35, 0x0066CC, "LFSR", 1, SW_Frame);
			draw_button(667, 365, 120, 35, 0x0066CC, "QPSK", 0, SW_Frame);
		select_modulation(3);
	...
	if (x_mouse >= 296 && x_mouse <= (296 + play_up.width) && y_mouse >= 45
		...
		DispIMAGE_from_gimp(SW_Frame, &pause_up, 523, 45, BLACK_24);
		DispIMAGE_from_gimp(SW_Frame, &back_up, 333, 177, BLACK_24);
		DispIMAGE_from_gimp(SW_Frame, &ford_down, 512, 177, BLACK_24);
